...
Routes requests to the correct provider based on asset type:
  ...
  Stock / ETF   → fx_provider     (Twelve Data — real prices, 15-min delayed on free plan)
  Forex         → fx_provider     (Twelve Data — real-time)
  Commodity     → fx_provider     (Twelve Data — real-time)
  Crypto spot   → crypto_provider (KuCoin — real-time)
...
Classification rules:
  ...
  - Everything else              → fx_provider (stocks)
...
class MarketDataRouter:
    ...
    async def get_current_price(self, symbol: str) -> float:
        ...
        log.debug("Routing → TwelveData (stock)", symbol=symbol)
        return await self._fx.get_current_price(symbol)
    ...
    async def get_quote(self, symbol: str) -> dict:
        ...
        if _is_forex_or_commodity(symbol):
            ...
            return q
        return await self._fx.get_quote(symbol)
    ...
    async def get_candles(self, symbol: str, timeframe: str, limit: int = 200):
        ...
        return await self._fx.get_candles(symbol, timeframe, limit)
    ...
    async def get_historical_candles(self, symbol: str, timeframe: str, start_date, end_date):
        ...
        return await self._fx.get_historical_candles(symbol, timeframe, start_date, end_date)
    ...
    async def update_price(self, symbol: str) -> float:
        ...
        return await self._fx.update_price(symbol)
...
def _create_router() -> MarketDataRouter:
    ...
    if has_key:
        log.info(
            "STOCK/FOREX/COMMODITY DATA SOURCE: TWELVEDATA (REAL)",
            api_key_loaded=True,
            TWELVEDATA_KEY_LOADED=True,
        )
    else:
        log.warning(
            "STOCK/FOREX/COMMODITY DATA SOURCE: GBM_SIMULATION (no API key)",
